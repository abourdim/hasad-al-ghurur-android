#!/usr/bin/env bash
# ============================================================================
#  Hasad al-Ghurur Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/25-hasad-al-ghurur}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.hasadalghurur}"
AVD_NAME="${AVD_NAME:-hasad_al_ghurur_test}"
...
cmd_test() {
  ...
  local out="/tmp/hasad-al-ghurur-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Hasad al-Ghurur Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
