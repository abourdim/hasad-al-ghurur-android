/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Hasad Al-Ghurur
...
(function() {
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Hasad Al-Ghurur/gi, 'Hassad al Rourour')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Hasad Al-Ghurur/gi, 'Hassad al Ghuroor')
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'حصاد الغرور' : l === 'fr' ? 'La Moisson de l\'Orgueil' : 'The Harvest of Arrogance',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('hg-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('hg-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('hg-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('hg-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('hg-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('hg-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('hg-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('hg-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('hg-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('hg-narrator-duo') === 'true';
  }
